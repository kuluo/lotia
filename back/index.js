...
exports.useCustomTableName = function (sequelize, sequelizeModel, value, isColumn = true, generatePostfixFunc, targetHooks) {
    ...
    beforeHooks.forEach(hookName => {
        ...
        sequelizeModel.addHook(hookName, HOOK_NAME, (entity, options) => {
            let realValue;
            if (isColumn) {
                // console.log('debug for intern', entity, value);
                realValue = entity[value];
                // console.log('debug for intern after', value);
            }
            realValue = generatePostfixFunc(realValue);
            sequelizeModel.tableName = sequelizeModel[BACK_TABLE_NAME_KEY] + "_" + realValue;
        });
    });
